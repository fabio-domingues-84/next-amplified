...
export default function UserCreateForm(props) {
  ...
  const [objectives, setObjectives] = React.useState(initialValues.objectives);
  ...
  const resetStateValues = () => {
    ...
    setCurrentObjectivesValue("");
    setDeleted(initialValues.deleted);
    setErrors({});
  };
  const [currentObjectivesValue, setCurrentObjectivesValue] =
    React.useState("");
  const objectivesRef = React.createRef();
  const validations = {
    name: [],
    username: [],
    gender: [],
      ...
      <ArrayField
        onChange={async (items) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            values = result?.objectives ?? values;
          }
          setObjectives(values);
          setCurrentObjectivesValue("");
        }}
        currentFieldValue={currentObjectivesValue}
        label={"Objectives"}
        items={objectives}
        hasError={errors?.objectives?.hasError}
        runValidationTasks={async () =>
          await runValidationTasks("objectives", currentObjectivesValue)
        }
        errorMessage={errors?.objectives?.errorMessage}
        setFieldValue={setCurrentObjectivesValue}
        inputFieldRef={objectivesRef}
        defaultFieldValue={""}
      >
        <TextField
          label="Objectives"
          isRequired={false}
          isReadOnly={false}
          value={currentObjectivesValue}
          onChange={(e) => {
            let { value } = e.target;
            if (errors.objectives?.hasError) {
              runValidationTasks("objectives", value);
            }
            setCurrentObjectivesValue(value);
          }}
          onBlur={() =>
            runValidationTasks("objectives", currentObjectivesValue)
          }
          errorMessage={errors.objectives?.errorMessage}
          hasError={errors.objectives?.hasError}
          ref={objectivesRef}
          labelHidden={true}
          {...getOverrideProps(overrides, "objectives")}
        ></TextField>
      </ArrayField>
      <SwitchField
        label="Deleted"
